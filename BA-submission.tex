...
\runtitle{The tenets of indirect inference in Bayesian models}

\begin{aug}
\author{Dmytro Perepolkin\thanksref{lund,t1}\ead[label=ea-1,email]{dmytro.perepolkin@cec.lu.se}}, \author{Benjamin Goodrich\thanksref{columbia}}, \author{Ullrika Sahlin\thanksref{lund}\ead[label=ea-3,email]{ullrika.sahlin@cec.lu.se}}
\runauthor{Perepolkin et al.}

 %loop through affiliations
\address[lund]{Centre for Environmental and Climate Science, Lund University, Sölvegatan 37, 223 62 Lund, Sweden \ifstrequal{lund}{lund}{\printead{ea-1}}{}
  \ifstrequal{lund}{lund}{\printead{ea-3}}{}
%authors
} %loop through affiliations
...
\thankstext{t1}{Corresponding Author: dmytro.perepolkin@cec.lu.se}
...
\end{keyword}
% keywords
 \begin{keyword} \kwd{bayesian analysis} \kwd{quantile functions} \kwd{indirect inference}\end{keyword}
\end{frontmatter}
...
\begin{supplement}
\stitle{Stan and R code}
\sdescription{Target markdown \citep{landau2021TargetsPackageDynamic} with corresponding targets defined in R and Stan for executing all examples in the paper.}
\end{supplement}
\begin{supplement}
\stitle{Moebius strip of probability functions}
\sdescription{DIY souvenir illustrating the relationship between the probability functions.}
\end{supplement}
...
\bibliography{BA-submission.bib}
