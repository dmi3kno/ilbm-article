...
```{r gamma-ray-prior-graph, echo=FALSE, fig.width=7, fig.height=5, fig.cap="Prior distribution of parameter \u03bb", out.width="50%", fig.align='center'}
ggplot()+
  ...
  scale_color_identity(name="Priors", 
    ...
    labels=c("Gamma(4,1e-3)", paste0("Rayleigh(", claims_data$ray_sigma, ")")),
    guide = "legend")
```
...
```{r rexp-prior-lik-graphs, out.width="50%", fig.cap="Summary of the posterior samples from the Rayleigh-Exponential model with direct and indirect prior and likelihood", fig.align='center'}
...
The Figure \@ref(fig:newton-animation-graph) illustrates the numerical approximation of probability $u$ corresponding to the observed claim amount equal to $100$ in the Claims Example given the sampling model $Exponential(0.002)$. We provide the probability guess of 0.5 as pretty ignorant initial value. The algorithm required 4 iterations to arrive at the approximation of the true probability of `r pexp(100,0.002)` given the desired tolerance, which for the purposes of this example we set at $10^{-3}$.

```{r, newton-animation-graph, out.width="50%", fig.cap="Approximating the probability  in the Exponential model using Newton-Rhapson method", fig.align='center', message=FALSE, warning=FALSE, echo=FALSE}
pdf(file="img/newton-graph.pdf", width=14, height=10)
par(mfrow=c(2,2))
animation::newton.method(FUN=function(u) 100 - (-log(1-u)/0.002), init=0.5, rg=c(0,1), tol = 1e-3,
...
```{r, fig.width=14, fig.height=5, fig.align='center', echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Chebyshev polynomial approximation for validating two quantile distributions"}
ps_grd <- seq(0,1,1e-3)
