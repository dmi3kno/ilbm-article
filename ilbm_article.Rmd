...
knitr::opts_chunk$set(echo = FALSE, dev=c('png', 'cairo_pdf'), dpi = 200, fig.width=8, fig.height=5)
...
```{r qdist-chart, fig.cap="Probability distributions, quantile distributions and parameterization.", fig.align='center'}
...
```{r moebius-chart, fig.cap="Moebius strip of probability functions.", fig.align='center'}
...
da_gexp_pdf_pdf <- tar_read(fit_gexp_pdf_pdf) %>% 
  ...
  posterior::rename_variables("direct likelihood"="lambda")

da_gexp_pdf_dqf <- tar_read(fit_gexp_pdf_dqf) %>% 
  posterior::as_draws_array() %>% 
  posterior::subset_draws(variable="lambda") %>% 
  posterior::rename_variables("indirect likelihood"="lambda")
```
...
posterior::bind_draws(da_gexp_pdf_pdf, da_gexp_pdf_dqf) %>% 
  ...
  kbl(booktabs=TRUE, linesep = "", col.names=c("model", "mean", "median", "q5", "q95", "rhat"),
      digits = c(NA, 4,4,4,4,3),
      caption = "Posterior sample comparison for the models") %>% 
  kable_paper(full_width = FALSE, latex_options =c("hold_position"))
```

```{r gexp-likelihood-graphs, fig.cap="Distribution of posterior samples from the gamma-exponential model with direct and indirect likelihood", out.width="80%", fig.align='center', fig.width=8, fig.height=5}
posterior::bind_draws(da_gexp_pdf_pdf, da_gexp_pdf_dqf) %>% 
  ...
  labs(title="Gamma-Exponential model",
       subtitle="Comparing posterior distributions of \u03bb from models with direct and indirect likelihoods",
       x="\u03bb", color="Model", fill="Model",
...
```{r gamma-ray-prior-graph, echo=FALSE, fig.width=8, fig.height=5, fig.cap="Substitution prior for the rate parameter", out.width="80%", fig.align='center'}
ggplot()+
...
```{r, fig.show="hold", out.width="80%", fig.width=8, fig.height=5, echo=FALSE}
...
da_rexp_pdf_pdf <- tar_read(fit_rexp_pdf_pdf) %>% 
  posterior::as_draws_array() %>% 
  posterior::subset_draws(variable="lambda") %>% 
  posterior::rename_variables("direct prior, direct likelihood"="lambda")
```
...
da_rexp_pdf_dqf <- tar_read(fit_rexp_pdf_dqf) %>% 
  posterior::as_draws_array() %>% 
  posterior::subset_draws(variable="lambda") %>% 
  posterior::rename_variables("direct prior, indirect likelihood"="lambda")
```


```{r, fig.show="hold", out.width="80%", fig.width=8, fig.height=7, echo=FALSE}
#bayesplot::mcmc_combo(raydqf_fit, pars = c("lambda"))
#posterior::summarise_draws(raydqf_fit)
da_rexp_dqf_pdf <-tar_read(fit_rexp_dqf_pdf) %>% 
  posterior::as_draws_array() %>% 
  posterior::subset_draws(variable="lambda") %>% 
  posterior::rename_variables("indirect prior, direct likelihood"="lambda")
```
...
da_rexp_dqf_dqf <- tar_read(fit_rexp_dqf_dqf) %>% 
  posterior::as_draws_array() %>% 
  posterior::subset_draws(variable="lambda") %>% 
  posterior::rename_variables("indirect prior, indirect likelihood"="lambda")
```
...
posterior::bind_draws(
  ...
  kbl(booktabs=TRUE, linesep = "", col.names = c("model", "mean", "median", "q5", "q95", "rhat"),
      digits=c(NA, 4,4,4,4,3),
      caption = "Summary of the posterior samples of the parameter lambda from the Rayleigh-Exponential model with direct and indirect prior and likelihood") %>% 
  kable_paper(full_width = FALSE, latex_options =c("hold_position"))
```
...
```{r rexp-prior-lik-graphs, out.width="80%", fig.cap="Distribution of the posterior samples from the Rayleigh-Exponential model with direct and indirect prior and likelihood", fig.align='center'}
posterior::bind_draws(
  da_rexp_pdf_pdf,
  da_rexp_pdf_dqf,
  da_rexp_dqf_pdf,
...
```{r bathtub-hist, echo=FALSE, out.width="80%", fig.cap="Histogram of the time to failure data", fig.width=8, fig.height=5, fig.align='center'}
...
We adopt the generalized exponential prior for the parameter $\gamma$ of Govindarajulu distribution with hyperparameters $\alpha$ `r paste(" =", bathtub_data['genexp_alpha'])` and $\lambda$ `r paste(" =", bathtub_data['genexp_lambda'])`. The parameter $\sigma$ of the Govindarajulu distribution can not be lower than the maximum of the observed times-to-failure. Therefore, we defined a shifted exponential prior with the rate $\lambda$ `r paste(" =", bathtub_data['genexp_lambda'])` and varying lower bound corresponding to the highest time to failure (in our example equal to `r bathtub_data$min_sigma`). The Jacobian for adding a constant to the sampled parameter $\sigma$, although required, is equal to one, so there is no impact on the log density, as the shifting transform produces a Jacobian derivative matrix with a unit determinant.

```{r govi-tab, echo=FALSE}
tar_load(fit_gexpgovi_pdf_dqf_as) 

np_gexpgovi_pdf_dqf_as <- bayesplot::nuts_params(fit_gexpgovi_pdf_dqf_as)
lp_gexpgovi_pdf_dqf_as <- bayesplot::log_posterior(fit_gexpgovi_pdf_dqf_as)

fit_gexpgovi_pdf_dqf_as %>% 
  posterior::as_draws_array() %>% 
  posterior::subset_draws(variable=c("gamma", "sigma")) %>% 
  posterior::summarise_draws() %>% .[,c(1:3, 6:8)] %>% 
  kbl(booktabs=TRUE, linesep = "", col.names = c("parameter", "mean", "median", "q5", "q95", "rhat"),
      digits=c(NA, 4,4,4,4,3),
      caption = "Summary of the posterior samples from the GenExp-Govindarajulu model using indirect likelihood") %>% 
  kable_paper(full_width = FALSE, latex_options =c("hold_position"))
```

```{r govi-pairs-graph, echo=FALSE, fig.width=10, fig.height=7, out.width="80%", fig.cap="Posterior distribution of parameters in GenExp-Govindarajulu model", fig.align='center'}
posterior::as_draws_array(fit_gexpgovi_pdf_dqf_as) %>% 
    bayesplot::mcmc_pairs(pars=c("gamma", "sigma", "lp__"),
                        lp = lp_gexpgovi_pdf_dqf_as,
                        np = np_gexpgovi_pdf_dqf_as,
                        condition = pairs_condition(nuts = "accept_stat__"),
                        off_diag_args = list(size = 1, alpha = 0.02))
```

```{r govi-nuts-graph, echo=FALSE, fig.width=14, fig.height=5, out.width="100%", fig.cap="NUTS parameters for GenExp-Govindarajulu model", fig.align='center', message=FALSE, warning=FALSE}
p2_govi <- bayesplot::mcmc_nuts_acceptance(np_gexpgovi_pdf_dqf_as, lp_gexpgovi_pdf_dqf_as)
p3_govi <- bayesplot::mcmc_nuts_divergence(np_gexpgovi_pdf_dqf_as, lp_gexpgovi_pdf_dqf_as)
gridExtra::grid.arrange(p2_govi, p3_govi, nrow=1)
```
...
```{r pgnh-code, eval=FALSE, echo=TRUE}
...
We take a random sample of `r nrow(dat_gnh_df)` values from the *g-and-h* distribution with parameters A=`r A`, B=`r B`, C=`r C`, g=`r g` and h=`r h`. The distribution of simulated values is shown in Figure \@ref(fig:gnh-data).

```{r gnh-data, echo=FALSE, fig.width=8, fig.height=5, fig.cap="Simulated data from g-and-h distribution", out.width="80%", fig.align='center'}
ggplot(dat_gnh_df)+
...
```{r gnh-combo-graph, fig.width=10, fig.height=7, out.width="80%", fig.align='center', fig.cap="Posterior distribution and traceplot of parameters in g-and-h distribution"}
fit_gnh %>% 
...
```{stan output.var="none", eval=FALSE, echo=TRUE}
model {
  vector[N] u;
...
```{r chebyshev-roots-gnk-graph, fig.width=14, fig.height=5, fig.align='center', echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Chebyshev polynomial approximation: valid QDF and true-positive proxy roots"}
ps_grd <- seq(0,1,1e-3)
i_from=1e-12; i_to=1-1e-12
cC <- qpd::get_chebyshev_coeffs(qpd::fgnk, n=10, from=i_from, to=i_to, A=5, B=5, C=0.8, g=5, k=-0.38)
roots <- qpd::find_chebyshev_roots(cC, from=i_from, to=i_to)
roots_df <- tibble(x=roots, y=0)
mid_rs_df <- tibble(x=stats::na.omit(stats::filter(c(0, roots, 1), rep(1, 2), sides=1)/2),
                    y=0,
                    valid=qpd::fgnk(x, A=5, B=5, C=0.8, g=5, k=-0.38) > 0)
...
p1 <- tibble(p=ps_grd, "function"=y, "approximation *"=y_est) %>%
  ...
  labs(title = "Invalid quantile distribution",
       subtitle="gnk(5, 5, 0.8, 5, -0.38), no partitioning", color=NULL, y="q(p)",
       caption="* approximated with Chebyshev polynomial with n=10")

part_grid <- c(0,make_pgrid(10, s=2),1)
part_df <- tibble(from=head(part_grid, -1), to=tail(part_grid, -1)) %>% 
  mutate(cC=map2(from, to, ~qpd::get_chebyshev_coeffs(qpd::fgnk, n=3, 
                            from=.x, to=.y, A=5, B=5, C=0.8, g=5, k=-0.38)),
         root_x=pmap(list(cC, from, to), qpd::find_chebyshev_roots),
         root_y=0.0,
         y_est=pmap(list(cC, from, to), ~tibble(x=seq(..2, ..3, length.out=50),
                                                y=drop(qpd::eval_chebyshev_poly(..1, x, from=..2, to = ..3))))
         )
part_roots_df <-unnest(part_df, root_x)
part_mid_rs_df <- tibble(x=stats::na.omit(stats::filter(c(0, part_roots_df$root_x, 1), rep(1, 2), sides=1)/2),
                    y=0,
                    valid=qpd::fgnk(x, A=5, B=5, C=0.8, g=5, k=-0.38) > 0)

p2 <- unnest(part_df, y_est) %>% 
  select(x,y) %>% 
  rename("approximation *"=y) %>% 
  mutate("function"=qpd::fgnk(x, A=5, B=5, C=0.8, g=5, k=-0.38)) %>% 
  tidyr::pivot_longer(-x) %>% 
  dplyr::mutate(name=forcats::fct_rev(name)) %>% 
  ggplot()+
    geom_vline(data=enframe(part_grid),aes(xintercept=value), lty=2, size=0.25, color="grey80")+
  geom_line(aes(x=x, y=value, color=name))+
  geom_point(data=part_roots_df, aes(x=root_x, y=root_y), pch=1,size=2, color="firebrick")+
  scale_color_manual(values=c("function"=viridisE6colors[1], "approximation *"=viridisE6colors[5]))+
  coord_cartesian(ylim = c(-0.5,500))+
  theme_ipsum_rc(grid=FALSE)+
  labs(title = "Invalid quantile distribution",
       subtitle="gnk(5, 5, 0.8, 5, -0.38), 10 partitions", color=NULL, y="q(p)",
       caption="* approximated with Chebyshev polynomials with n=3, grid s=2")
p1+p2
```
...
```{r chebyshev-roots-gnh-graph, fig.width=14, fig.height=5, fig.align='center', echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Chebyshev polynomial approximation: valid QDF and false-positive proxy roots"}

cC_v <- qpd::get_chebyshev_coeffs(qpd::fgnh, n=10, from=i_from, to=i_to, A=5, B=5, C=0.8, g=5, h=0.25)
...
p3<- tibble(p=ps_grd, "function"=y_v, "approximation *"=y_est_v) %>%
  tidyr::pivot_longer(-p, names_to = "name", values_to="value") %>% 
  dplyr::mutate(name=forcats::fct_inorder(name)) %>% 
  ggplot()+
  geom_line(aes(x=p, y=value, color=name))+
  geom_point(data=roots_df_v, aes(x=x, y=y), pch=1,size=2, color="royalblue")+
  scale_color_manual(values=c("function"=viridisE6colors[2], "approximation *"=viridisE6colors[5]))+
  theme_ipsum_rc(grid=FALSE)+
  coord_cartesian(ylim = c(-50,5000))+
  labs(title = "Valid quantile distribution",
       subtitle="gnh(5, 5, 0.8, 5, 0.25), no partitioning", color=NULL, y="q(p)",
       caption="* approximated with Chebyshev polynomial with n=10")


part_grid_v <- c(0,make_pgrid(10, s=2),1)
part_df_v <- tibble(from=head(part_grid_v, -1), to=tail(part_grid_v, -1)) %>% 
  mutate(cC=map2(from, to, ~qpd::get_chebyshev_coeffs(qpd::fgnh, n=3, 
                            from=.x, to=.y, A=5, B=5, C=0.8, g=5, h=0.25)),
         root_x=pmap(list(cC, from, to), qpd::find_chebyshev_roots),
         root_y=0.0,
         y_est=pmap(list(cC, from, to), ~tibble(x=seq(..2, ..3, length.out=50),
                                                y=drop(qpd::eval_chebyshev_poly(..1, x, from=..2, to = ..3))))
         )
part_roots_df_v <- unnest(part_df_v, root_x)
p4 <- unnest(part_df_v, y_est) %>% 
  select(x,y) %>% 
  rename("approximation *"=y) %>% 
  mutate("function"=qpd::fgnh(x, A=5, B=5, C=0.8, g=5, h=0.25)) %>% 
  tidyr::pivot_longer(-x) %>% 
  dplyr::mutate(name=forcats::fct_rev(name)) %>% 
  ggplot()+
    geom_vline(data=enframe(part_grid_v),aes(xintercept=value), lty=2, size=0.25, color="grey80")+
  geom_line(aes(x=x, y=value, color=name))+
  geom_point(data=part_roots_df_v, aes(x=root_x, y=root_y), pch=1,size=2, color="royalblue")+
  scale_color_manual(values=c("function"=viridisE6colors[1], "approximation *"=viridisE6colors[5]))+
  coord_cartesian(ylim = c(-50,5000))+
  theme_ipsum_rc(grid=FALSE)+
  labs(title = "Valid quantile distribution",
       subtitle="gnh(5, 5, 0.8, 5, 0.25), 10 partitions", color=NULL, y="q(p)",
       caption="* approximated with Chebyshev polynomial with n=3, grid s=2")
p3+p4
```
